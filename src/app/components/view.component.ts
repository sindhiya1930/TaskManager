...
<div class="input-group mb-3">
  ...
  <div class="row">
      ...
      <tbody>
          ...
          <th scope="col">   </th>
          <td><button type="button" class="btn" [disabled]="Task.Flag" (click)="edit(Task._id)">Edit</button></td>
          <td><button type="button" class="btn" [disabled]="Task.Flag" (click)="end(Task._id,Task.Task,Task.Priority,Task.Parent_Task,Task.Start_Date,Task.End_Date)"  >End Task</button></td>
          <td>{{message}}</td>
          </tr>
          </tbody>
          </table>
